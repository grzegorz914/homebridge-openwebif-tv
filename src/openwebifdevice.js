...
class OpenWebIfDevice extends EventEmitter {
    ...
    async prepareAccessory(mac) {
        try {
            ...
            this.televisionService.getCharacteristic(Characteristic.TargetMediaState)
                ...
                .onSet(async (value) => {
                    try {
                        // [fix] removed unused newMediaState variable
            ...
            if (this.volumeControl > 0) {
                // [fix] removed redundant volumeServiceNameTv — both were identical
                ...
                switch (this.volumeControl) {
                    ...
                    case 2: // fan
                        ...
                        break;
                    case 3: // tv speaker
                        this.volumeServiceTvSpeaker = this._setupTvSpeakerService(accessory, volumeServiceName);
                        break;
            ...
            if (maxInputsSwitchesButtonsCount > 0) {
                ...
                for (let i = 0; i < maxInputsSwitchesButtonsCount; i++) {
                    ...
                    const rawReference = button.reference;
                    const reference = encodeURIComponent(rawReference);
                    ...
                    inputButtonService.getCharacteristic(Characteristic.On)
                        .onGet(async () => {
                            return inputButtonService.state;
                        })
                        .onSet(async (state) => {
                            if (!state) return;
                            ...
                            try {
                                switch (mode) {
                                    ...
                                    case 2: //Power Control
                                        ...
                                        if (this.logDebug) this.emit('debug', `Set Power Control, Name: ${name}, Reference: ${command}`);
                                        button.state = false;
                                        break;
                                    default:
                                ...
                                if (this.logWarn) this.emit('warn', `Set ${['Channel', 'Command', 'Power'][mode] ?? 'Unknown'} error: ${error}`);
                            }
                        });
    ...
    _setupTvSpeakerService(accessory, volumeServiceName) {
        if (this.logDebug) this.emit('debug', `Prepare television speaker service`);
        const svc = accessory.addService(Service.TelevisionSpeaker, volumeServiceName, 'TV Speaker');
        ...
        svc.getCharacteristic(Characteristic.VolumeSelector)
            .onSet(async (command) => {
                try {
                    command = command === Characteristic.VolumeSelector.INCREMENT ? '115' : '114';
                    await this.openwebif.send(ApiUrls.SetRcCommand + command);
            ...
            this.openwebif = new OpenWebIf(this.device, this.devInfoFile, this.inputsFile, this.channelsFile, this.mqtt.enable)
                ...
                .on('stateChanged', async (power, name, eventName, reference, volume, mute, recording, streaming, playState) => {
                    ...
                    const previousStateModeMap = {
                        ...
                        4: this.recording,
                        5: this.streaming,
                        // [fix] was this.playState (undefined) — now correctly reads instance property
                        6: this.playState,
                    };
                    ...
                    for (let i = 0; i < this.sensors.length; i++) {
                        ...
                        const isActiveMode = power || sensor.mode >= 4;
                    ...
                    for (let i = 0; i < this.buttons.length; i++) {
                        const button = this.buttons[i];
                        // [fix] was this.power (stale/old value) — now uses local `power` parameter
                        const state = power ? (button.mode === 1 ? (button.command === 'MUTE' ? muteV : button.command === 'POWER' ? power : button.state) : button.reference === reference) : false;
                        button.state = state;
